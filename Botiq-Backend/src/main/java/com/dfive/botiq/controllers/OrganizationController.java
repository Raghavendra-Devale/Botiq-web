...
//     @PostMapping("/check-user")
//     public ResponseEntity<Map<String, String>> checkUserExists(@RequestBody Map<String, String> payload) {
//     System.out.println("Received payload: " + payload);
...
// }
    ...
    // public ResponseEntity<Map<String, Object>> createUser(@RequestBody CreateOrgUserDto requestDto) {
    //     Map<String, Object> response = new HashMap<>();
    ...
    //         Optional<Organization> existingOrgByMobile = organizationRepository.findByMobileNumber(requestDto.getMobileNumber());
    ...
    //             response.put("success", false);
    //             response.put("message", "Organization name cannot be null or empty when creating a new organization.");
    //             return ResponseEntity.status(HttpStatus.BAD_REQUEST).body(response);
    //         }
...
public ResponseEntity<Map<String, Object>> registerUser(
    ...
    try {
        ...
        Optional<OrgUser> existingUserByMobile = orgUserRepository.findByMobileNumber(requestDto.getMobileNumber().trim());
...
public ResponseEntity<Map<String, String>> sendEmailOtp(
    ...
    if (orguser.getEmailId() == null || orguser.getEmailId().trim().isEmpty()) {
        return ResponseEntity.status(HttpStatus.BAD_REQUEST).body(Map.of("message", "Email ID is not registered. Please contact the developer."));
    }
    ...
    return ResponseEntity.ok(Map.of("message", "OTP sent to your email.", "txnId", txnId, "maskedEmail", maskedEmail));
}
...

{
    ...
    if (optionalOtpTransaction.isEmpty()) {
        return ResponseEntity.status(HttpStatus.BAD_REQUEST).body(Map.of("success", false, "message", "Invalid OTP request."));
    }
    ...
    if (Instant.now().isAfter(otpTransaction.getRequestDate().plusSeconds(otpTransaction.getValidDuration()))) {
        return ResponseEntity.status(HttpStatus.BAD_REQUEST).body(Map.of("success", false, "message", "OTP expired."));
    }

    if (!passwordEncoder.matches(enteredOtp, otpTransaction.getEmailOtp())) {
        return ResponseEntity.status(HttpStatus.BAD_REQUEST).body(Map.of("success", false, "message", "Incorrect OTP."));
    }
    ...
    try {
        ...
        if (fid == null) {
            return ResponseEntity.status(500).body(Map.of("success", false, "message", "Failed to fetch Firebase UID."));
        }
        ...
        return ResponseEntity.status(500).body(Map.of("success", false, "message", "Failed to update Firebase claims."));
    }
